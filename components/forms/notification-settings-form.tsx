    ...
    <ActionForm form={form} className="flex flex-col gap-5">
      ...
      <fieldset className="flex flex-col gap-2 border-b border-border pb-5">
        ...
        {CHANNELS.map((c) => (
          <label
            key={c}
            // `min-h-6` and `py-1`: the whole label is the tap target, and a
            // 20px line of text is under the 24px minimum (e2e/touch-targets).
            className="flex min-h-6 items-start gap-3 py-1 text-sm"
          >
            <input
              type="radio"
              name="channel"
      ...
      >
        ...
        <fieldset className="flex flex-col gap-2">
          ...
          <div className="mt-1 grid gap-3 sm:grid-cols-2">
            {mode === "hoursBefore" && (
              <Field
                label={t("hoursBefore")}
                error={state.fieldErrors?.hoursBefore}
              >
                <Input
                  type="number"
                  min="1"
            ...
            {mode === "morningOf" && (
              <Field
                label={t("morningHour")}
                error={state.fieldErrors?.morningHour}
              >
                <Input
                  type="number"
                  min="0"
        ...
        <fieldset className="flex flex-col gap-3 rounded-control border border-border p-3">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <Field
              label={t("remindersDaysBefore")}
              error={state.fieldErrors?.remindersDaysBefore}
            >
              <Input
                type="number"
                min="0"
